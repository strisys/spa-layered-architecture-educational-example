...
> **Source:** Originally downloaded from https://github.com/robonuggets/marp-slides (SKILL.md + `examples/` folder) on 2026-04-21.

## Prerequisites
- VS Code extension "Marp for VS Code"
- Settings: `markdown.marp.enableHtml: true` + `markdown.marp.allowLocalFiles: true`
- Export: `npx @marp-team/marp-cli slides.md --pdf --html --allow-local-files`

> **Gotcha 1:** The VS Code `enableHtml` setting only affects the VS Code preview. For CLI exports (PDF/PPTX/HTML) you MUST pass `--html` or inline SVG and other HTML tags will be silently stripped/mangled (e.g., `stroke-width` becomes `strokewidth`, svg appears as literal text in the PDF). Alternatively, commit a project-level `.marprc.yml` with `html: true` so exports pick it up automatically.
>
> **Gotcha 2 — blank lines inside HTML blocks:** markdown-it ends an HTML block at the first blank line. A blank line between two HTML tags (including inside `<svg>` or between sibling `<div>`s) will terminate the block; the remainder gets reparsed as markdown and raw tags leak into the PDF. **Keep HTML content contiguous** — no blank lines between `</...>` and the next `<...>`, no blank lines between `<!-- comment -->` groups inside an SVG. Use blank lines only at the boundary where HTML ends and markdown (or `---`) resumes.
...
The `--html` flag is REQUIRED whenever a deck contains inline `<svg>`, `<div>`, or other raw HTML (which every dark-theme deck built from this skill does). Without it, Marp's default sanitizer strips attributes and leaks tag contents as literal text. Prefer a committed `.marprc.yml` (`html: true`) so the flag can't be forgotten.
